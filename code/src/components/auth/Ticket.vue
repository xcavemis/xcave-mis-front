<template>
    <div class="ticket-comp">
        ...
        <form class="ticket-comp__form">
            ...
            <button class="default-button black ticket-comp__begin-bt" type="submit" @click="sendData" :disabled="!isValid">CONTINUAR</button>
        </form>
        <div class="ticket-comp__register-disclaimer">
          <div>Ainda não tem ingresso<br>para experiência digital?</div>
          <a class="default-button white" href="https://www.sympla.com.br/teste-mis-davinci-digital-v1__904158" target="_blank">COMPRAR INGRESSO DIGITAL</a>
        </div>
    </div>
</template>
...
export default {
  ...
  methods: {
    show(){
      ...
      const splittingSubtitleName = Splitting({ target: this.$el.querySelector('.auth__subtitle-name'), by: 'words' })
      const splittingSubtitle = Splitting({ target: this.$el.querySelector('.auth__subtitle-desc'), by: 'words' })
