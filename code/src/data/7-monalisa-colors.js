export const monalisaColors = {
    scenes: [
      {
        id: "monalisa-colors701",
        groupId: 'monalisa',
        src: 'MONA-LISA-COLORS-701',
        type: "image",
        name: "monalisa-colors 701",
        faceSize: 6144,
        initialViewParameters: {
          vec3: [-4881.56, -369.82, 1004.78],
        },
        infoHotspots: [
          {
            vec3: [-702.60, -1184.48, -4799.51],
            target: "military606",
            title: "PROJECTION 606",
            type: 'link',
            direction: {
              vec3: [-2415.22, -267.32, -4361.55],
            },
          },
          {
            vec3: [2384.27, -1701.46, -4045.20],
            target: "monalisa-colors702",
            title: "MONA LISA COLORS 702",
            type: 'link',
            direction: {
              vec3: [4327.57, -161.56, -2491.05],
            },
          },
          {
            vec3: [-4126.63, -720.28, -2713.92],
            title: "D-701",
            type: 'content',
            image: "101/la-bele-feroniere.jpg",
            audio: '101/guide',
            text: "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Suspendisse sapien elit, fermentum aliquet malesuada tempus, aliquet sodales magna. Phasellus ultrices mattis bibendum.Lorem ipsum dolor sit amet, consectetur adipiscing elit. Suspendisse sapien elit, fermentum aliquet malesuada tempus, aliquet sodales magna. Phasellus ultrices mattis bibendum.Lorem ipsum dolor sit amet, consectetur adipiscing elit. Suspendisse sapien elit, fermentum aliquet malesuada tempus, aliquet sodales magna. Phasellus ultrices mattis bibendum.Lorem ipsum dolor sit amet, consectetur adipiscing elit. Suspendisse sapien elit, fermentum aliquet malesuada tempus, aliquet sodales magna. Phasellus ultrices mattis bibendum.Lorem ipsum dolor sit amet, consectetur adipiscing elit. Suspendisse sapien elit, fermentum aliquet malesuada tempus, aliquet sodales magna. Phasellus ultrices mattis bibendum."
          },
        ]
      },
      {
        id: "monalisa-colors702",
        groupId: 'monalisa',
        src: 'MONA-LISA-COLORS-702',
        type: "image",
        name: "monalisa-colors 702",
        faceSize: 6144,
        initialViewParameters: {
          vec3: [2232.73, -230.52, -4456.16]
        },
        infoHotspots: [
          {
            vec3: [-4346.77, -2254.45, -994.53],
            target: "monalisa-colors701",
            title: "MONA LISA COLORS 701",
            type: 'link',
            direction: {
              vec3: [-4968.85, -479.59, 103.14],
            },
          },
          {
            vec3: [-1712.54, -1607.61, 4404.56],
            target: "monalisa-colors703",
            title: "MONA LISA COLORS 703",
            type: 'link',
            direction: {
              vec3: [1760.62, -511.10, 4641.02],
            },
          },
          {
            vec3: [4042.05, 294.44, -2922.38],
            title: "D-702",
            type: 'content',
            image: "101/la-bele-feroniere.jpg",
            audio: '101/guide',
            text: "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Suspendisse sapien elit, fermentum aliquet malesuada tempus, aliquet sodales magna. Phasellus ultrices mattis bibendum.Lorem ipsum dolor sit amet, consectetur adipiscing elit. Suspendisse sapien elit, fermentum aliquet malesuada tempus, aliquet sodales magna. Phasellus ultrices mattis bibendum.Lorem ipsum dolor sit amet, consectetur adipiscing elit. Suspendisse sapien elit, fermentum aliquet malesuada tempus, aliquet sodales magna. Phasellus ultrices mattis bibendum.Lorem ipsum dolor sit amet, consectetur adipiscing elit. Suspendisse sapien elit, fermentum aliquet malesuada tempus, aliquet sodales magna. Phasellus ultrices mattis bibendum.Lorem ipsum dolor sit amet, consectetur adipiscing elit. Suspendisse sapien elit, fermentum aliquet malesuada tempus, aliquet sodales magna. Phasellus ultrices mattis bibendum."
          },
          {
            vec3: [4957.16, -278.31, -550.41],
            title: "D-703",
            type: 'content',
            image: "101/la-bele-feroniere.jpg",
            audio: '101/guide',
            text: "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Suspendisse sapien elit, fermentum aliquet malesuada tempus, aliquet sodales magna. Phasellus ultrices mattis bibendum.Lorem ipsum dolor sit amet, consectetur adipiscing elit. Suspendisse sapien elit, fermentum aliquet malesuada tempus, aliquet sodales magna. Phasellus ultrices mattis bibendum.Lorem ipsum dolor sit amet, consectetur adipiscing elit. Suspendisse sapien elit, fermentum aliquet malesuada tempus, aliquet sodales magna. Phasellus ultrices mattis bibendum.Lorem ipsum dolor sit amet, consectetur adipiscing elit. Suspendisse sapien elit, fermentum aliquet malesuada tempus, aliquet sodales magna. Phasellus ultrices mattis bibendum.Lorem ipsum dolor sit amet, consectetur adipiscing elit. Suspendisse sapien elit, fermentum aliquet malesuada tempus, aliquet sodales magna. Phasellus ultrices mattis bibendum."
          },
          {
            vec3: [4283.36, -293.07, 2550.91],
            title: "D-704",
            type: 'content',
            image: "101/la-bele-feroniere.jpg",
            audio: '101/guide',
            text: "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Suspendisse sapien elit, fermentum aliquet malesuada tempus, aliquet sodales magna. Phasellus ultrices mattis bibendum.Lorem ipsum dolor sit amet, consectetur adipiscing elit. Suspendisse sapien elit, fermentum aliquet malesuada tempus, aliquet sodales magna. Phasellus ultrices mattis bibendum.Lorem ipsum dolor sit amet, consectetur adipiscing elit. Suspendisse sapien elit, fermentum aliquet malesuada tempus, aliquet sodales magna. Phasellus ultrices mattis bibendum.Lorem ipsum dolor sit amet, consectetur adipiscing elit. Suspendisse sapien elit, fermentum aliquet malesuada tempus, aliquet sodales magna. Phasellus ultrices mattis bibendum.Lorem ipsum dolor sit amet, consectetur adipiscing elit. Suspendisse sapien elit, fermentum aliquet malesuada tempus, aliquet sodales magna. Phasellus ultrices mattis bibendum."
          },
          {
            vec3: [2240.92, -201.29, 4453.27],
            title: "D-705",
            type: 'content',
            image: "101/la-bele-feroniere.jpg",
            audio: '101/guide',
            text: "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Suspendisse sapien elit, fermentum aliquet malesuada tempus, aliquet sodales magna. Phasellus ultrices mattis bibendum.Lorem ipsum dolor sit amet, consectetur adipiscing elit. Suspendisse sapien elit, fermentum aliquet malesuada tempus, aliquet sodales magna. Phasellus ultrices mattis bibendum.Lorem ipsum dolor sit amet, consectetur adipiscing elit. Suspendisse sapien elit, fermentum aliquet malesuada tempus, aliquet sodales magna. Phasellus ultrices mattis bibendum.Lorem ipsum dolor sit amet, consectetur adipiscing elit. Suspendisse sapien elit, fermentum aliquet malesuada tempus, aliquet sodales magna. Phasellus ultrices mattis bibendum.Lorem ipsum dolor sit amet, consectetur adipiscing elit. Suspendisse sapien elit, fermentum aliquet malesuada tempus, aliquet sodales magna. Phasellus ultrices mattis bibendum."
          },
        ]
      },
      {
        id: "monalisa-colors703",
        groupId: 'monalisa',
        src: 'MONA-LISA-COLORS-703',
        type: "image",
        name: "monalisa-colors 703",
        faceSize: 6144,
        initialViewParameters: {
          vec3: [1760.62, -511.10, 4641.02]
        },
        infoHotspots: [
          {
            vec3: [-1666.96, -1770.31, -4357.85],
            target: "monalisa-colors702",
            title: "MONA LISA COLORS 702",
            type: 'link',
            direction: {
              vec3: [2152.79, -230.88, -4496.87],
            },
          },
          {
            vec3: [-588.84, -1715.90, 4650.98],
            target: "monalisa-colors704",
            title: "MONA LISA COLORS 704",
            type: 'link',
            direction: {
              vec3: [-4616.70, -778.68, -1735.71],
            },
          },
          {
            vec3: [3097.60, -314.92, -3900.69],
            title: "D-706",
            type: 'content',
            image: "101/la-bele-feroniere.jpg",
            audio: '101/guide',
            text: "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Suspendisse sapien elit, fermentum aliquet malesuada tempus, aliquet sodales magna. Phasellus ultrices mattis bibendum.Lorem ipsum dolor sit amet, consectetur adipiscing elit. Suspendisse sapien elit, fermentum aliquet malesuada tempus, aliquet sodales magna. Phasellus ultrices mattis bibendum.Lorem ipsum dolor sit amet, consectetur adipiscing elit. Suspendisse sapien elit, fermentum aliquet malesuada tempus, aliquet sodales magna. Phasellus ultrices mattis bibendum.Lorem ipsum dolor sit amet, consectetur adipiscing elit. Suspendisse sapien elit, fermentum aliquet malesuada tempus, aliquet sodales magna. Phasellus ultrices mattis bibendum.Lorem ipsum dolor sit amet, consectetur adipiscing elit. Suspendisse sapien elit, fermentum aliquet malesuada tempus, aliquet sodales magna. Phasellus ultrices mattis bibendum."
          },
          {
            vec3: [4850.14, -198.21, -1161.03],
            title: "D-707",
            type: 'content',
            image: "101/la-bele-feroniere.jpg",
            audio: '101/guide',
            text: "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Suspendisse sapien elit, fermentum aliquet malesuada tempus, aliquet sodales magna. Phasellus ultrices mattis bibendum.Lorem ipsum dolor sit amet, consectetur adipiscing elit. Suspendisse sapien elit, fermentum aliquet malesuada tempus, aliquet sodales magna. Phasellus ultrices mattis bibendum.Lorem ipsum dolor sit amet, consectetur adipiscing elit. Suspendisse sapien elit, fermentum aliquet malesuada tempus, aliquet sodales magna. Phasellus ultrices mattis bibendum.Lorem ipsum dolor sit amet, consectetur adipiscing elit. Suspendisse sapien elit, fermentum aliquet malesuada tempus, aliquet sodales magna. Phasellus ultrices mattis bibendum.Lorem ipsum dolor sit amet, consectetur adipiscing elit. Suspendisse sapien elit, fermentum aliquet malesuada tempus, aliquet sodales magna. Phasellus ultrices mattis bibendum."
          },
          {
            vec3: [3888.35, 254.68, 3116.59],
            title: "D-708",
            type: 'content',
            image: "101/la-bele-feroniere.jpg",
            audio: '101/guide',
            text: "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Suspendisse sapien elit, fermentum aliquet malesuada tempus, aliquet sodales magna. Phasellus ultrices mattis bibendum.Lorem ipsum dolor sit amet, consectetur adipiscing elit. Suspendisse sapien elit, fermentum aliquet malesuada tempus, aliquet sodales magna. Phasellus ultrices mattis bibendum.Lorem ipsum dolor sit amet, consectetur adipiscing elit. Suspendisse sapien elit, fermentum aliquet malesuada tempus, aliquet sodales magna. Phasellus ultrices mattis bibendum.Lorem ipsum dolor sit amet, consectetur adipiscing elit. Suspendisse sapien elit, fermentum aliquet malesuada tempus, aliquet sodales magna. Phasellus ultrices mattis bibendum.Lorem ipsum dolor sit amet, consectetur adipiscing elit. Suspendisse sapien elit, fermentum aliquet malesuada tempus, aliquet sodales magna. Phasellus ultrices mattis bibendum."
          },
        ]
      },
      {
        id: "monalisa-colors704",
        groupId: 'monalisa',
        src: 'MONA-LISA-COLORS-704',
        type: "image",
        name: "monalisa-colors 704",
        faceSize: 6144,
        initialViewParameters: {
          vec3: [2771.93, -323.67, 4138.39]
        },
        infoHotspots: [
          {
            vec3: [4628.98, -1623.98, -944.22],
            target: "monalisa-colors703",
            title: "MONA LISA COLORS 703",
            type: 'link',
            direction: {
              vec3: [-519.03, -502.83, -4944.30],
            },
          },
          {
            vec3: [-3838.04, -1262.29, 2932.84],
            target: "monalisa-colors705",
            title: "MONA LISA COLORS 705",
            type: 'link',
            direction: {
              vec3: [-1065.84, 147.83, 4877.69],
            },
          },
          {
            vec3: [4079.61, -53.32, 2881.92],
            title: "D-709",
            type: 'content',
            image: "101/la-bele-feroniere.jpg",
            audio: '101/guide',
            text: "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Suspendisse sapien elit, fermentum aliquet malesuada tempus, aliquet sodales magna. Phasellus ultrices mattis bibendum.Lorem ipsum dolor sit amet, consectetur adipiscing elit. Suspendisse sapien elit, fermentum aliquet malesuada tempus, aliquet sodales magna. Phasellus ultrices mattis bibendum.Lorem ipsum dolor sit amet, consectetur adipiscing elit. Suspendisse sapien elit, fermentum aliquet malesuada tempus, aliquet sodales magna. Phasellus ultrices mattis bibendum.Lorem ipsum dolor sit amet, consectetur adipiscing elit. Suspendisse sapien elit, fermentum aliquet malesuada tempus, aliquet sodales magna. Phasellus ultrices mattis bibendum.Lorem ipsum dolor sit amet, consectetur adipiscing elit. Suspendisse sapien elit, fermentum aliquet malesuada tempus, aliquet sodales magna. Phasellus ultrices mattis bibendum."
          },
        ]
      },
      {
        id: "monalisa-colors705",
        groupId: 'monalisa',
        src: 'MONA-LISA-COLORS-705',
        type: "image",
        name: "monalisa-colors 705",
        faceSize: 6144,
        initialViewParameters: {
          vec3: [4894.20, -266.41, 950.88]
        },
        infoHotspots: [
          {
            vec3: [910.07, -1508.46, -4674.26],
            target: "monalisa-colors704",
            title: "MONA LISA COLORS 704",
            type: 'link',
            direction: {
              vec3: [3935.08, -88.34, -3069.26],
            },
          },
          {
            vec3: [-1070.04, -1428.74, 4663.49],
            target: "monalisa-layers801",
            title: "MONA LISA LAYERS 801",
            type: 'link',
            direction: {
              vec3: [3098.07, -24.21, 3914.99],
            },
          },
          {
            vec3: [4365.06, -315.56, 2404.17],
            title: "D-710",
            type: 'content',
            image: "101/la-bele-feroniere.jpg",
            audio: '101/guide',
            text: "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Suspendisse sapien elit, fermentum aliquet malesuada tempus, aliquet sodales magna. Phasellus ultrices mattis bibendum.Lorem ipsum dolor sit amet, consectetur adipiscing elit. Suspendisse sapien elit, fermentum aliquet malesuada tempus, aliquet sodales magna. Phasellus ultrices mattis bibendum.Lorem ipsum dolor sit amet, consectetur adipiscing elit. Suspendisse sapien elit, fermentum aliquet malesuada tempus, aliquet sodales magna. Phasellus ultrices mattis bibendum.Lorem ipsum dolor sit amet, consectetur adipiscing elit. Suspendisse sapien elit, fermentum aliquet malesuada tempus, aliquet sodales magna. Phasellus ultrices mattis bibendum.Lorem ipsum dolor sit amet, consectetur adipiscing elit. Suspendisse sapien elit, fermentum aliquet malesuada tempus, aliquet sodales magna. Phasellus ultrices mattis bibendum."
          },
        ]
      },
    ],
  };
  